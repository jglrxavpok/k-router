...
class RouteMatcherTest {
    ...
    fun `static matching with single route`() {
        ...
        assertEquals("route", matcher.match("/foo", GET).route.name)
    }
    ...
    fun `static matching with multiple routes with same path but different method`() {
        ...
        assertEquals("route1", matcher.match("/foo", GET).route.name)
    }
    ...
    fun `one of the routes is chosen when two routes match`() {
        ...
        assert(listOf("route1", "route2").contains(matcher.match("/foo", GET).route.name))
    }
    ...
    fun `GET and HEAD methods are equivalent`() {
        ...
        assertEquals("route2", matcher.match("/foo", HEAD).route.name)
    }
    ...
    fun `multiple dynamic routes with different requirements but same path`() {
        ...
        assertEquals("route1", matcher.match("/foo/3/bar", GET).route.name)
        assertEquals("route2", matcher.match("/foo/abcd/bar", GET).route.name)
    }
    ...
    fun `dynamic route parameters are populated`() {
        ...
        assertEquals("route1", match.route.name)
    ...
    fun `dynamic route with multiple parameters`() {
        ...
        assertEquals("route1", matcher.match("/foo/3/bar/test", GET).route.name)
